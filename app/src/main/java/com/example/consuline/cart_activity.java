...
public class cart_activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(list.isEmpty()){
            ...
            for(Model m : list){
                Price+=Integer.parseInt(m.getCost().substring(0,m.getCost().indexOf("k")).trim())*m.getQuant();
                cartlist.put(m.getId(),m.getQuant());
            }
            //Toast.makeText(this,Integer.toString(adapter.totalprice),Toast.LENGTH_LONG).show();
            totalPrice.setText(Integer.toString(Price)+"k");
...
//            add.setText(address);
    ...
    private void Order() {
        ...
        String url = "http:///localhostORipadress/temp/getorderid.php";
